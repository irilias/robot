...
public class MoveCommandStrategy(Domain.Robot robot) : ICommandStrategy
{
    private readonly Domain.Robot _robot = robot;

    public bool CanHandle(string command) => string.Equals(command, CommandConstants.Move, StringComparison.OrdinalIgnoreCase);
