...
public class StatusCommandStrategy(Domain.Robot robot) : ICommandStrategy
{
    private readonly Domain.Robot _robot = robot;

    public bool CanHandle(string command) => string.Equals(command, CommandConstants.Status, StringComparison.OrdinalIgnoreCase);
